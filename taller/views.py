...
from .backward_motor import (  # Importamos los datos globales
    HECHOS,
    REGLAS,
    backward_chaining,
)
...
class ReglaForm(forms.Form):
    ...
    explicacion = forms.CharField(
        label="Explicación", widget=forms.Textarea, required=False
    )
...
def index(request):
    ...
    return render(request, "index.html", {"hechos": hechos, "es_admin": es_admin_flag})
...
def diagnosticar(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "diagnostico.html",
            {"resultado": resultado, "es_admin": es_admin_flag},
        )
...
def backward_inicio(request):
    ...
    conclusiones = list(
        set(r["conclusion"] for r in REGLAS)
    )  # usa la lista de reglas del backward_motor.py
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "backward_resultado.html",
            {"confirmado": confirmado, "mensaje": mensaje, "objetivo": objetivo},
        )

    return render(
        request,
        "backward_inicio.html",
        {"hechos": HECHOS, "conclusiones": conclusiones},
    )
            ...
            if regla_form.is_valid():
                ...
                index = next(
                    (
                        ...
                        if r["condicion"] == cond and r["conclusion"] == conc
                    ),
                    None,
                )
                if index is not None:
                    ...
                    reglas.append(
                        {"condicion": cond, "conclusion": conc, "explicacion": expl}
                    )
                return redirect("administrar_backward")

    return render(
        request,
        "administrar_backward.html",
        {
            "hecho_form": hecho_form,
            "regla_form": regla_form,
            "hechos": hechos,
...
def eliminar_regla_backward(request, condicion, conclusion):
    ...
    thismodule.REGLAS = [
        r
        for r in reglas
        if not (r["condicion"] == condicion and r["conclusion"] == conclusion)
    ]
    return redirect("administrar_backward")
